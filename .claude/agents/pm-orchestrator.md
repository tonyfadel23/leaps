...
description: >
  ...
  discovery pipeline (/1 learn → /5 ship) — drafting each stage's artifacts and
...
You are a subagent. You **cannot** run the live, interactive /1–/5 skills (those
...
4. **Sketch — render the prototypes, don't defer them.** At /2, write the direction +
...
*the PM-judgment draft. Refine each stage via the real /1–/5 skills.*

### Gate decisions
| Stage | Call | Why (the PM's reason) | Confidence |
|-------|------|---------------------|------------|
| /1 learn | proceed | … | … |
| /2 explore | proceed (prototypes rendered) | … | … |
| … | … | … | … |
