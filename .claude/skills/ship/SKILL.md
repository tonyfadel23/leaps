...
baselines, and a feasibility-cleared direction. `/5` turns that into
...
Or after completing `/4`, confirm the handoff: "Run `/5` to continue."
...
**Staleness check:** Same pattern as /2-/4. Check all upstream artifacts
...
2. **Verify readiness**:
   ...
   - If Red: "Feasibility is Red — resolve blocking constraints before building.
     Run `/4` to address [specific blocker]."
   - Kill criteria must be defined in assess.md
   - Recommended direction must be selected in prove.md
   - If any are missing: "Run `/3` and `/4` first — I need defined metrics and
...
**Iteration loop**: Same as /2 — PM can request changes, additions, or
...
3. **Append to `pipeline/{idea-slug}/decisions.md`**:
   - Append `### /5 Ship — {date}` to the Decision Log section
...
Extract `briefData` fields per `_shared/reference/brief-field-mappings.md` (/5 section).
...
- **Prototype Builder**: `../explore/agents/prototype-builder.md` — Reused from /2.
...
**Protocols:**
- **Handoff Schema**: `_shared/protocols/handoff-schema.md` — Validates /4→/5 contract on entry

---

## Related Skills

- `/4 prove` — Previous step: feasibility and recommended direction
