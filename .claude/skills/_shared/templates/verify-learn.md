...
Structural checks on /learn outputs. Fast pass/fail — flag failures but don't block.
...
- [ ] "Done looks like" is one sentence with no metric numbers (those come in /assess)
...
**On failure:** Flag which checks failed. Don't block — these gaps may surface in `/explore`.
