...
- Update `decisionLog` — append /assess Assess entries
- Update `openQuestions` and `decisions`

### General updates:

- Update `meta.stage` to `'Assessed'`, `meta.date` to today
- `feasibility: null` — not available until /prove
...
From `decisions.md` (updated):
- Update `decisionLog` — append /prove Prove entries
- Update `openQuestions` and `decisions`
...
- Update `decisionLog` — append /ship Ship entries.
