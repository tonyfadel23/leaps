...
**Critical — Pull before you probe:**

The Interviewer has a **mandatory occasion validation** (step 3b) that MUST
run EARLY — right after the occasion is named, BEFORE asking a string of
opinion questions or generating outcomes. Attempt the data/knowledge pull
first; only then ask the PM to fill the specific blanks it leaves. Skipping
this is the most common failure mode: an outcome map full of `[inferred]`
scores and an interview that loops without ever converging.

**Backstop:** If an outcome map still shows >30% `[inferred]` scores, stop
and ask: "Most of these scores are inferences — should I pull data first?"
The occasion validation requires:
...
**Competitor research is NOT part of grounding.** It happens in `/explore`
...
competitor research in /explore targets your top outcomes.
...
**Converge on readiness, not a question count.** Drive toward a **phase readiness**
score per `_shared/protocols/phase-convergence.md` — there is **no fixed question
budget**. `/learn`'s readiness rubric (its required outputs, each source-tagged, gaps
named): **occasion named + core job named + ≥2 scored outcomes + a rough size + a
complexity read.** Each turn, self-assess readiness (coverage × grounding-honesty) and
report it as `phaseReadiness` in the turn's `leap-findings`. When readiness ≥ 80 — or it
plateaus with no path to raise it — STOP asking, present the Approval Gate, state the
readiness read, and recommend `/explore`. Pull before you probe (grounding lifts readiness
faster than opinion questions). Never loop on data you can't ground: if readiness can't
rise, present what you have with the gaps named and let the PM confirm or redirect.

**Zero-connector fast path:** If no data/knowledge connectors are configured, run
the single graceful-degradation pull (Data Analyst returns the sizing formula with
blanks; Context Retriever returns "grounding from conversation only"), ask the PM
**once** for any numbers they have, tag results `[pm]`/`[inferred]`, note the gap,
and proceed to convergence. Do **not** keep asking data questions you cannot ground.
...
3. In Phase 5, suggest running the research plan before `/explore`
...
The interview has **converged** when **phase readiness ≥ 80** — i.e. occasion + core
job + ≥2 scored outcomes + rough size + complexity are captured and honestly grounded
(some may be PM-provided or flagged as gaps). Don't wait for perfect data; converge on
...
**Template rules specific to /learn:**
...
Each claim includes: `metric`, `value`, `source`, `confidence`, `skill` ("/learn"), `date`.
...
Extract `briefData` fields per `_shared/reference/brief-field-mappings.md` (/learn section).
...
and outcome scores. More slides will appear as you run /explore, /assess, and /prove."
...
- **Green**: "This is well-grounded. Run `/explore` to sketch the experience."
- **Yellow**: "We have gaps in [X]. I'd suggest [specific action] before moving on. Want to dig deeper, or move to `/explore` anyway?"
- **Yellow with research plan**: "We have no direct customer evidence for this problem. I've created a research plan — see `research-plan.md`. Consider running it before `/explore` to validate the problem is real. Want to proceed anyway, or run the research first?"
- **Red**: "The problem isn't sharp enough yet. I'd recommend [specific question to answer]. Want to continue grounding?"

If the PM confirms next step, state: "Run `/explore` to continue." Do not auto-chain.

### Phase 6 — Collect Feedback

Follow `_shared/protocols/feedback-collection.md` with skill name `/learn`. This protocol runs at two cadences: **per-phase micro-ratings** after each phase completes (§A — one tap, non-blocking) and the **skill-end rating + follow-up** (§B). All feedback saves to `pipeline/{idea-slug}/eval/feedback.md`.
...
- `/explore` — Next step: lo-fi prototypes and journey maps
- `/assess` — Success criteria, KRs, targets
- `/prove` — Feasibility, constraints, competing directions
