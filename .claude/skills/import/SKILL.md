...
| Has | Covers Stage |
|-----|-------------|
| JTBD + occasion + outcomes + sizing | /1 learn |
| + prototypes or journey map or variations | /2 explore |
| + metric tree + kill criteria + baselines | /3 assess |
| + feasibility + risks + dependencies | /4 prove |
| + release checklist or build plan | /5 ship |

Show the PM:
```
This document covers through /3 assess:
...
Import as "{slug}" at stage /3? (or adjust)
```
...
```
Imported to /3 assess. Gaps found:
...
/2 Explore gaps:
  - No HTML prototypes (run /2 to generate)
...
Recommended next step:
  ...
  /4 {slug}            — continue to feasibility (fill gaps later)
  /2 {slug} --phase prototype  — generate prototypes from the JTBD
```
...
- `/1 learn` — Start from scratch instead of importing
