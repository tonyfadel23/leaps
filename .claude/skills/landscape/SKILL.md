...
description: >
  ...
  for downstream /1 learn processing. Use when a PM says "/landscape", "map this
...
an entire domain, plus scaffolded pipeline directories ready for `/1 learn`.
...
*Next step: Run `/1 learn {bet-slug}` on individual bets to sharpen problem statements*
```
...
1. **For each bet in the landscape**, create:
   ...
   └── .state.json           # { "skill": "/landscape", "next_skill": "/1 learn" }
   ```
   ...
   Run `/1 learn {bet-slug}` to start problem grounding.
   ```

2. **Don't overwrite** — if a pipeline directory already exists for a bet
   (e.g., from prior `/1 learn` work), skip it and note: "Skipped `{slug}` — already exists."
...
3. **Present scaffold summary**:
   ...
   Run `/1 learn {slug}` on any bet to start detailed problem grounding.
...
- **Does not replace `/1 learn`** — the landscape is a strategic overview.
...
- **Does not interview the PM deeply** — it has two approval gates (structure, sizing)
  but doesn't run a full conversational interview. That's `/1 learn`'s job.
- **Does not create prototypes** — that's `/2 explore`.
- **Does not define success metrics** — that's `/3 assess`.
