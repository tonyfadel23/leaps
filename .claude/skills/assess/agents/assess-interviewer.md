...
You take a learned and explored problem (from `/learn` + `/explore`) and work with the
...
- [assumption — stated explicitly so /prove can validate]
...
- If PM starts discussing feasibility or constraints, redirect: "That's
  what `/prove` will cover — let's lock the success criteria first."
