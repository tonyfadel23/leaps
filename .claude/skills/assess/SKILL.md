...
description: >
  ...
  Use when a PM says "/3" or "/assess", or after completing /2 (explore). Reads
...
Take a prototyped experience (from `/2 explore`) and define success criteria.
...
Or after completing `/2`, confirm the handoff: "Run `/3` to continue."
...
data to the PM: "From /1 learn, I have these baselines: [list]. Want me to
...
4. **Confidence check**: If ground confidence is **red**, warn:
   ...
   [X] remains an assumption. Consider revisiting `/1` first."

Present to the PM:
- "Here's where we are: [JTBD, chosen direction, opportunity size]"
- "The outcome map from /1 gives us [N] scored outcomes. The top [M] underserved
...
- **Decision log** path: `pipeline/{idea-slug}/decision-log.md` — agent
  appends Q&A under `## /3 Assess — {date}`, continuing from last number

The Interviewer:
- Detects the PM's starting point (open, has conviction, has data)
- **Inherits the outcome table** from /1 learn as a pre-built metric tree seed:
  ...
  - **Baselines** = the Satisfaction scores from /1 ARE the baselines — S=2 on a
...
4. **Append to `pipeline/{idea-slug}/decisions.md`** — add /3 Assess
   decisions:
   - Append `### /3 Assess — {date}` to the Decision Log section
...
Extract `briefData` fields per `_shared/reference/brief-field-mappings.md` (/3 section).
...
Present: "The brief now includes success criteria and build scope. Run `/4`
...
- "Success criteria are set. Run `/4` to explore feasibility, constraints, and
...
Follow `_shared/protocols/feedback-collection.md` with skill name `/3 assess`. This protocol runs at two cadences: **per-phase micro-ratings** after each phase completes (§A — one tap, non-blocking) and the **skill-end rating + follow-up** (§B). All feedback saves to `pipeline/{idea-slug}/eval/feedback.md`.

---

## Principles

1. **Inherited, not invented.** The outcome table from /1 learn IS the metric
   tree seed. North Star = highest-Opp outcome. Inputs = next 3-5 underserved
   outcomes. Baselines = satisfaction scores. /3's job is to operationalize —
   mapping each outcome to a trackable metric and setting the satisfaction target.
   Don't reinvent what /1 already scored.
...
3. **Sketch-informed.** The experience direction from `/2` shapes which metrics
...
7. **Measurable, not aspirational.** KRs have baselines (satisfaction scores
   from /1), targets (satisfaction improvement), confidence levels, and
...
8. **No feasibility yet.** If the PM starts asking about technical constraints
   or team dependencies, redirect: "That's exactly what `/4` will cover.
...
- `/2 explore` — Previous step: lo-fi prototypes and journey maps
- `/4 prove` — Next step: feasibility, constraints, competing directions
