...
description: >
  ...
  or after completing /learn (learn). No feasibility, no constraints, no metrics —
...
Take a sharp problem (from `/learn`) and make it tangible before anyone
...
Or after completing `/learn`, confirm the handoff: "Run `/explore` to continue."
...
re-run later with `/explore {slug} --competitors`), or **Manual input** (PM pastes
...
citation tags. Append a footer: `*Researched by /explore, {date}*`.
...
`pipeline/{idea-slug}/decision-log.md` under a `## /explore Explore — {date}`
header. Continue numbering from where `/learn` left off.
...
5. **Append to `pipeline/{idea-slug}/decisions.md`** — add /explore Explore decisions.

**Update the pipeline brief:**

Extract `briefData` fields per `_shared/reference/brief-field-mappings.md` (/explore section).
...
Recommend next step: "The experience is sketched. Run `/assess` to define success criteria and targets informed by what we just built."

### Phase 6 — Collect Feedback

Follow `_shared/protocols/feedback-collection.md` with skill name `/explore`. This protocol runs at two cadences: **per-phase micro-ratings** after each phase completes (§A — one tap, non-blocking) and the **skill-end rating + follow-up** (§B). All feedback saves to `pipeline/{idea-slug}/eval/feedback.md`.
...
3. **No constraints.** Don't filter ideas through feasibility, campaigns,
   existing systems, or technical limitations. That's `/prove`'s job.
...
- `/learn` — Previous step: JTBD problem statement and sizing
- `/assess` — Next step: success criteria, KRs, targets informed by sketches
- `/prove` — Grind sketches against reality: feasibility, constraints, directions
