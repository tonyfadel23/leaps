...
description: >
  ...
  Use when a PM says "/4" or "/prove", or after completing /assess (assess).
...
explicitly deferred feasibility and implementation direction here. `/prove` delivers
...
**No constraints in `/explore`. No feasibility in `/assess`. That work starts here.**
...
Or after completing `/assess`, confirm the handoff: "Run `/prove` to continue."
...
2. **Audit assumptions**: Extract every assumption from all prior files. These
   are the inputs that `/prove` is responsible for validating or qualifying.
   Look for:
   ...
   - Open questions from explore.md and assess.md that were deferred to `/prove`
   - Gaps flagged in the Build context table (No / Partial entries)
   - Blocking questions in the "Decide before building" section

3. **Validate handoff schema**: Check the /assess→/4 handoff contract
...
5. **Present audit summary to PM**:
   - "I've read all prior outputs. Here's what `/prove` needs to resolve: [N assumptions, N open questions, N 'No' Build context rows]"
...
`## /prove Prove — {date}`, continuing from the last numbered question.
...
- Decision log path: `pipeline/{idea-slug}/decision-log.md` — agent appends
  Q&A under the existing `## /prove Prove — {date}` header
...
This is the approach that goes into the opportunity brief and drives /ship.
Confirm?"
...
2. **If a target is invalidated, revise it in both files:**
   - `assess.md` — update the number in the Success Criteria table. Add
     a footnote: `*Revised from {original} — see decision-log.md /prove Prove*`
...
3. **Log the revision** in `decision-log.md` under the existing
   `## /prove Prove — {date}` header:

   ```markdown
   **Target revised** (When: Cascade)
   - Metric: {metric name}
   - Original target: {original value} (set in /assess)
...
3. **Append to `pipeline/{idea-slug}/decisions.md`** — add /prove Prove
   decisions:
   - Append `### /prove Prove — {date}` to the Decision Log section
   - Update Open Questions and Resolved Decisions tables

**Update the pipeline brief:**

Extract `briefData` fields per `_shared/reference/brief-field-mappings.md` (/prove section).
...
- **Green**: "Feasibility is clear. The recommended direction is [X]. Run `/ship` to build
  the production prototype."
- **Yellow**: "We have open dependencies: [list]. I'd suggest [specific action] before
  starting `/ship`. Want to resolve these, or proceed anyway?"
...
Follow `_shared/protocols/feedback-collection.md` with skill name `/prove`. This protocol runs at two cadences: **per-phase micro-ratings** after each phase completes (§A — one tap, non-blocking) and the **skill-end rating + follow-up** (§B). All feedback saves to `pipeline/{idea-slug}/eval/feedback.md`.
...
1. **First skill where solutions are on the table.** Every prior skill deferred
   implementation questions here. `/prove` holds that boundary — don't apologize for it.
...
4. **Interrogate assess.md assumptions.** The metric tree and targets from `/assess`
...
8. **Leave `/ship` a clean handoff.** The recommended direction should tell an engineer
   or AI agent exactly which implementation approach was chosen, which dependencies
   are resolved, and which open questions remain. If `/ship` has to re-ask feasibility
   questions, `/prove` failed.
...
- **Handoff Schema**: `_shared/protocols/handoff-schema.md` — Validates /assess→/4 contract on entry
...
- `/explore` — Lo-fi prototypes and journey maps
- `/assess` — Success criteria, KRs, targets, kill criteria
- `/ship` — Next step: production-ready prototype and release checklist
