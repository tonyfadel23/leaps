...
(() => {
  ...
  let mountedView = null, mountedSession = null, lastBriefSig = null, lastMsgCount = 0, lastBoardSig = null, lastConvSig = null, lastOverlaySig = null;
  ...
  function sessionPartial(host, s) {
    ...
    if (liveScroll && freshScroll && convSig(s) === lastConvSig) freshScroll.replaceWith(liveScroll);
  ...
  function render() {
    ...
    // Keep the conversation pinned to the latest message as discovery streams in.
    const msgCount = s.idea && s.idea.messages ? s.idea.messages.length : 0;
    if (s.view === 'session' && msgCount !== lastMsgCount) {
      const scroll = document.querySelector('.conv-scroll');
  ...
  async function boot() {
    ...
    const [board, pipelineDir, status] = await Promise.all([DS.getBoard(), DS.getPipelineDir(), DS.getStatus()]);
    Store.set({ board, pipelineDir, loading: false });
