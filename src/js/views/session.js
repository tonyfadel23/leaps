...
window.SessionView = (() => {
  ...
  const MODELS = [
    ...
    { id: 'fable',  label: 'Fable',   sub: 'Latest, most capable' },
    { id: 'opus',   label: 'Opus',    sub: 'Highly capable' },
  ...
  function modelShort(id) {
    ...
    const fam = s.match(/^(fable|opus|sonnet|haiku)/i);
    const ver = s.match(/(\d+)(?:[-.](\d+))?/);
    const oneM = /\[1m\]|(?:^|[^a-z0-9])1m(?:[^a-z0-9]|$)/i.test(s) ? ' 1M' : '';
    if (fam) {
      const name = fam[1][0].toUpperCase() + fam[1].slice(1);
      return (ver ? name + ' ' + ver[1] + (ver[2] ? '.' + ver[2] : '') : name) + oneM;
    }
    return s.replace(/-\d{6,}$/, '') + oneM;
  }
  ...
  // Footer card when a discovery run finishes — what moved. (Per-turn cost is
  // intentionally not shown here; aggregate usage lives in /pipeline --cost.)
  ...
  function modelMenu(s) {
    const cur = s.model || '';
    // Base family aliases + the org's own cached model options (pinned versions,
    // 1M-context variants) so the picker follows what this account actually offers.
    const dyn = ((s && s.modelOptions) || []).map(o => ({ id: o.value, label: o.label, sub: o.description || '' }));
    const seen = new Set();
    const list = MODELS.concat(dyn).filter(m => (seen.has(m.id) ? false : (seen.add(m.id), true)));
