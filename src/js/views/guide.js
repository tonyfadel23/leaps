...
window.GuideView = (() => {
  ...
  const AGENTS = [
    ...
    ['pm-orchestrator', 'A first-pass /1→/5 run with senior-PM judgment.'],
  ...
  const PRACTICES = [
    ...
    ['Re-run upstream freely', 'Discovery is non-linear. Re-running /1 marks downstream artifacts stale; the pipeline tracks what needs refreshing.'],
  ...
  function overview() {
    return `
      ...
      <p class="gd-p">Everything runs on your machine and drives your own Claude. Each concept is a folder of plain files. Start a concept with <b>+ New idea</b> or <code>/1</code>, then just type.</p>`;
  }
  function pipeline() {
    return `<p class="gd-p">Five chainable stages. Each is conversation-first and ends by recommending the next. Run them as <code>/1</code>…<code>/5</code> or by name.</p>
