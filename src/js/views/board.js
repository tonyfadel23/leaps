...
window.BoardView = (() => {
      ...
      <div class="conn-role ${bound ? 'bound' : 'unbound'}">
        <div class="conn-role-h">
          <div class="conn-role-id">
            <span class="conn-dot ${bound ? 'ok' : 'mut'}"></span>
          ...
          <span class="conn-state ${bound ? 'on' : ''}">${bound ? 'Connected' : 'Not connected'}</span>
        </div>
  ...
  function connectorsEditor(s) {
    ...
    return `
      ...
      <div class="conn-list">${draft.map(connRoleBlock).join('')}</div>
