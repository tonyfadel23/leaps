...
**Phase routing:**
```
/learn water-subscription --phase tribal     # re-run tribal knowledge pull
/learn water-subscription --phase sizing     # re-pull sizing data
/learn water-subscription --phase synthesize # rewrite learn.md from existing Q&A
/learn water-subscription --phase verify     # re-run structural checks only
```
...
**Phase routing:**
```
/explore water-subscription --phase prototype    # rebuild prototypes from confirmed concepts
/explore water-subscription --phase converge     # re-converge on a direction
/explore water-subscription --phase verify       # re-run structural checks only
```
...
**Protocols:** Handoff Schema (/learn→/2 validation)
...
**Phase routing:**
```
/assess water-subscription --phase validate     # re-pull baselines from data sources
/assess water-subscription --phase synthesize   # rewrite assess.md from existing interview
/assess water-subscription --phase verify       # re-run structural checks only
```
...
**Phase routing:**
```
/prove water-subscription --phase scout       # re-run architecture scout only
/prove water-subscription --phase interview   # restart feasibility interview
/prove water-subscription --phase synthesize  # rewrite prove.md from findings
/prove water-subscription --phase verify      # re-run structural checks only
```
...
**Protocols:** Handoff Schema (/assess→/4 validation), Consistency Check (mid-interview), Claims Cache (read/write)
...
**Phase routing:**
```
/ship water-subscription --phase stress-test   # re-run stress test only
/ship water-subscription --phase checklist     # regenerate release checklist
/ship water-subscription --phase verify        # re-run structural checks
```
...
**Agents:**
- Prototype Builder (reused from /explore) — mid-fi upgrade with error states, loading states
...
**Protocols:** Handoff Schema (/prove→/5 validation)
...
Requires at least `/learn` completed. Also auto-generated during `/explore`, `/assess`, `/prove` Synthesize phases.
...
| `auth-preflight.md` | Check MCP auth before skill start, cache results | Pipeline skills /learn-/5, /grill-me, /landscape, /compare |
| `data-analyst.md` | Sizing, baselines, instrumentation assessment | /learn, /assess, /prove, /ship, /landscape |
...
| `doc-analyzer.md` | Source document extraction (folder, dropped files, links) | /learn, /landscape |
