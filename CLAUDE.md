...
/learn water-subscription           # Start grinding an idea
...
When a PM opens a new conversation, run this sequence **before responding to their first message**. If the PM's first message is already a skill command (`/learn`, `/explore`, `/grill-me`, etc.), skip the welcome entirely and run the skill.
...
/learn    → Sharpen a fuzzy idea into a clear problem (JTBD)
/explore    → Prototype solutions before touching metrics
/assess    → Set success criteria, baselines, kill criteria
/prove   → Test feasibility, map dependencies, pick a direction
/ship     → Mid-fi prototype + stress test + release checklist

Tell me an idea, a problem, or a signal — or type /learn to start.
```
...
|---------------|-----------|
| Learned | `/explore` to explore solutions |
| Explored | `/assess` to assess success criteria |
| Assessed | `/prove` to prove feasibility |
| Proven | `/ship` to build mid-fi prototype |
...
- **Skill commands skip welcome.** If the PM types `/explore water-subscription` as their first message, go straight to the skill.
...
|---|-------|---------|--------|
| `/learn` | learn | Raw idea → sharp JTBD problem statement | Shipped |
| `/explore` | explore | Problem → lo-fi HTML prototypes + journey maps | Shipped |
| `/assess` | assess | Sketches → success criteria, KRs, baselines, kill criteria | Shipped |
| `/prove` | prove | KRs + sketches → feasibility, constraints, competing directions | Shipped |
| `/ship` | ship | Conviction → mid-fi prototype + stress-test + release checklist | Shipped |
...
│   └── pm-orchestrator.md  # First-pass /learn→/5 run with the PM's judgment (invocable)
...
│   │   │   ├── cascade.md           # Cascade invalidation (skills /learn-/5)
...
│   ├── learn/                  # /learn — interviewer, context retriever
│   ├── explore/                  # /explore — journey mapper, competitor researcher,
│   │                            #    prototype builder, showcase builder, comparative critique
│   ├── assess/                  # /assess — define interviewer
│   ├── prove/                 # /prove — architecture scout, feasibility interviewer,
│   │                            #    assumption auditor, direction generator, feature analyzer
│   ├── ship/                   # /ship — prototype builder (reused), stress tester
│   │   └── SKILL.md
...
└── pipeline/                    # Outputs from running skills
    ...
    └── {idea-slug}/
        ...
        ├── summary.md               # Brief page source: bet, evidence, seeking (/learn, updated by all)
        ├── outcomes.md              # Brief page source: job, occasion, outcome map (/learn)
        ├── competitors.md           # Brief page source: strategic read, differentiators (/explore)
        ├── journey.md               # Brief page source: direction, stages, intervention (/explore)
        ├── variations.md            # Brief page source: chosen + explored directions (/explore)
        ├── metrics.md               # Brief page source: north star, inputs, kill criteria (/assess)
        ├── scope.md                 # Brief page source: ship/don't/later + build context (/assess)
        ├── feasibility.md           # Brief page source: formula scoring, risks, deps (/prove)
        ...
        ├── learn.md                # /learn process output
        ├── research-plan.md         # /learn user research plan
        ├── competitors/             # /explore competitor research (raw analysis + screenshots)
        ...
        │   └── variation-*.html
        ├── prd.md                   # Vibe-coding handoff (/explore, updated by /prove)
        ├── assess.md                # /assess process output
        ├── prove.md               # /prove process output
