...
const SYSTEM_BRIEF = [
  ...
  '',
  'USING THE SKILLS — when the message is a slash command (e.g. `/learn`, `/explore`,',
  '`/assess`, `/prove`, `/ship`, `/grill-me`), invoke the matching LEAPs skill and',
  'follow it. For free-form messages, use the relevant skill\'s approach and judgment,',
  'but keep to a single exchange. Prefer the skills\' own file conventions and agents.',
  '',
  ...
  'this turn. If you established none, return an empty claims array.',
].join('\n');
...
function startDiscovery({ id, ideaDir, projectRoot, prompt, onEvent, onDone, mcpServers, promptBlock, model, sessionId, resume }) {
  ...
  const systemPrompt = SYSTEM_BRIEF
    + (ctx
